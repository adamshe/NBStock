...
namespace QuandlCS.Requests
{
  public class QuandlMultisetRequest : IQuandlRequest
  {
    ...
    private string CreateRequestString()
    {
      ...
      sb.Append(Constants.ApiAddress.APIMultisetsAddress)
        .Append(TypeConverter.FileFormatToString(_format))
        .Append('?')
        .Append(Constants.ApiProperty.APIColumns)
        .Append(_datacolumns.ToString())
        .Append('?');

      if (APIKey != string.Empty)
      {
        sb.Append(Constants.ApiProperty.APIAuthorization)
          .Append(APIKey)
          .Append('&');
      }

      sb.Append(Constants.ApiProperty.APIFrequency)
        .Append(TypeConverter.FrequencyToString(Frequency))
        .Append('&')
        .Append(Constants.ApiProperty.APITransformation)
        .Append(TypeConverter.TransformationsToString(Transformation))
        .Append('&')
        .Append(Constants.ApiProperty.APISortOrder)
        .Append(TypeConverter.SortOrdersToString(Sort));

      if (Truncation > 0)
      {
        sb.Append('&')
          .Append(Constants.ApiProperty.APITruncation)
          .Append(Truncation);
      }
      ...
      {
        sb.Append('&')
          .Append(Constants.ApiProperty.APIStartDate)
      ...
      {
        sb.Append('&')
          .Append(Constants.ApiProperty.APIEndDate)
      ...
      {
        sb.Append('&')
          .Append(Constants.ApiProperty.APIExcludeHeader)
          .Append(!Headers);
      }
