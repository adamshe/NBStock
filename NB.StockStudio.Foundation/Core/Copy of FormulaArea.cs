            ...
            if (this.BindingErrors != null)
            {
                ...
                try
                {
                    ...
                    for (int i = 0; i < FormulaDataArray.Count; i++)
                    {
                        ...
                        Pen currentPen = this.GetCurrentPen(i);
                        Font currentFont = this.GetCurrentFont(i);
                        ...
                        switch (formulaData.RenderType)
                        {
                            ...
                            case FormulaRenderType.VOLSTICK:
                                ...
                                data2 = this.Parent.DataProvider["CLOSE"];
                                if (this.StockRenderType != StockRenderType.Bar)
                                {
                                    break;
                                }
                                data3 = (FormulaData) (data2 > FormulaBase.REF(data2, 1.0));
                                goto Label_02F9;
                        ...
                        FormulaData data4 = this.DataProvider["OPEN"];
                        data3 = (FormulaData) (data2 > data4);
                    Label_02F9:
                        data3 *= formulaData.MaxValue(this.Canvas.Start, this.Canvas.Count);
                        data3.AxisY = this.AxisY;
                        data3.Canvas = this.Canvas;
                        tfArray = data3.GetPoints();
                        ...
                        continue;
                    Label_0501:
                        tf = new PointF(points[num7].X, num6);
                        g.DrawLine(penArray[(points[num7].Y < num6) ? 0 : 2], points[num7], tf);
                        num7++;
                    Label_054D:
                        if (num7 < points.Length)
                        {
                            goto Label_0501;
                        }
                        continue;
                    Label_055F:
                        ...
                        PointF tf2 = new PointF(points[num8].X, formulaData.AxisY.CalcY(numArray2[num8]));
                        g.DrawLine(currentPen, points[num8], tf2);
                        num8++;
                    Label_05B1:
                        if (num8 < points.Length)
                        {
                            goto Label_055F;
                        }
                        continue;
                    Label_06F0:
                        if (formulaData["COND"][num10] == 1.0)
                        {
                            g.DrawString(formulaData.OwnerData["ICON"].ToString(), new Font("Verdana", 9f), Brushes.Black, points[num10]);
                        }
                        num10++;
                    Label_074E:
                        if (num10 < points.Length)
                        {
                            goto Label_06F0;
                        }
                        continue;
                    Label_0760:
                        if (formulaData.Data[num11] == 1.0)
                        {
                            g.DrawLine(currentPen, points[num11].X, (float) R.Top, points[num11].X, (float) R.Bottom);
                        }
                        num11++;
                    Label_07AF:
                        if (num11 < points.Length)
                        {
                            goto Label_0760;
                        }
                        continue;
                    ...
                    Label_0BA1:
                        ...
                        else if (list.Count > 0)
                        {
                            ...
                            if (brush3 == null)
                            {
                                brush3 = new SolidBrush(Color.FromArgb(0x80, Color.Blue));
                            }
                            if (points.Length > 1)
                            {
                        ...
                        num18++;
                    Label_0C93:
                        if (num18 <= points.Length)
                        {
                            goto Label_0BA1;
                        }
                        continue;
                    Label_0D77:
                        if ((num19 == (points.Length - 1)) || points[num19].IsEmpty)
                        {
                            if (list3.Count > 0)
                            {
                        ...
                        {
                            list3.Add(points[num19]);
                        }
                        num19++;
                    Label_0DEC:
                        if (num19 < points.Length)
                        {
                            goto Label_0D77;
                        }
                        continue;
