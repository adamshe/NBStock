    ...
    public partial class YAccountManager
    {
        ...
        public bool LogOut()
        {
            if (this.IsLoggedIn)
            {
                ...
                Response<XParseDocument> resp = dl.Download();
                if (resp.Connection.State == ConnectionState.Success)
                {
                    mCookies = null;
                    this.SetCrumb(string.Empty);
                    var propertyChanged = Volatile.Read(ref this.PropertyChanged ) ;
                    if (propertyChanged != null) this.PropertyChanged(this, new System.ComponentModel.PropertyChangedEventArgs("IsLoggedIn"));
                }               
            }
            return this.IsLoggedIn;
        }
