...
namespace MaasOne.Base
{
    internal partial class TimeoutWebClient<T> : IDisposable
    {
            ...
            if (!mDisposedValue)
            {
                ...
                try
                {
                    if (postDataBytes != null)
                    {
                        ...
                        try
                        {
                            asyncArgs.Handle = System.Threading.ThreadPool.RegisterWaitForSingleObject(res.AsyncWaitHandle, new System.Threading.WaitOrTimerCallback(this.ResponseDownloadTimeout), asyncArgs, (this.Timeout), true);
                        }
        ...
        {
            var args = state as AsyncDownloadArgs<T>;
            if (timedOut)
            {
